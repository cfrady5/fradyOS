...
export async function getEventDetail(userId: string, id: string): Promise<EventDetail | null> {
  ...
  if (error) throw new Error(error.message);
  if (!ev) return null;
  const [tasks, posts, notes, atts, templates, items, conn] = await Promise.all([
    ...
    supabase.from("monday_connections").select("board_name,last_success_at,last_error").eq("user_id", userId).maybeSingle(),
  ]);
  return {
